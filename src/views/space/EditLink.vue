  ...
  <div class="container">
    <nav class="mt-2">
      <ol class="breadcrumb">
        ...
        <li v-if="loaded" v-show="rendered" class="breadcrumb-item">
          <router-link class="propertyNameCrumb" :to="`/space/${space.id}/property/${property.id}`"
            >Link</router-link
          >
        </li>
...
export default {
  ...
  mounted() {
    this.$http.get(`/api/space/${this.spaceId}/property/${this.propertyId}`).then(response => {
      ...
      this.editLink = this.clone(response.data.link)
      this.editReferences = this.clone(response.data.link.references)
