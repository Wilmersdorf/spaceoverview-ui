  ...
  <div class="container">
    <nav class="mt-2">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">
          <span v-if="loaded" v-show="rendered" class="math">{{ space.symbol }}</span>
        </li>
        ...
        >
          <div class="d-flex mt-2" v-if="linkProperty.field === fieldLink">
            ...
            <div v-if="showSource" class="d-flex">
              <div class="text-muted ml-1">(</div>
              <span class="text-muted" v-if="linkProperty.linked">linked</span>
              <span class="text-muted mr-1" v-if="linkProperty.linked && linkProperty.computed"
                >,</span
              >
...
export default {
  ...
  mounted() {
    this.$http.get(`/api/space/${this.id}`).then(spaceResponse => {
      ...
      this.$http.get(`/api/space/${this.id}/property`).then(propertyResponse => {
