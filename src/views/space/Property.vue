        ...
        <div v-for="linkSpace in linkSpaceList" :key="`${fieldLink}-${linkSpace.space.id}`">
          <div class="d-flex mt-2" v-if="linkSpace.field === fieldLink">
            ...
            <div v-if="showSource" class="d-flex">
              ...
              <span class="text-muted mr-1" v-if="linkSpace.linked && linkSpace.computed">,</span>
...
export default {
  ...
  mounted() {
    this.$http.get(`/api/property/${this.id}`).then(propertyResponse => {
      this.$http.get(`/api/property/${this.id}/space`).then(spaceResponse => {
        this.$http.get(`/api/theorem?propertyId=${this.id}`).then(theoremsResponse => {
